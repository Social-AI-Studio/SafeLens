...
- harm_categories: list of short strings (if any), each 1-2 words max
- factor_weights: object with modality weights in percentages; keys 'visual', 'audio', 'text' (each 0-100)
    ...
    try:
        ...
        if isinstance(result, dict):
            ...
            if "error" in result:
                ...
                logger.debug(
                    f"LLM decision for segment {segment_info}: harmful={is_harmful}, confidence={confidence:.2f}"
                )
                ...
                return {
                    ...
                    "_token_usage": result.get(
                        "_token_usage"
                    ),  # Preserve token usage if available
                }
        else:
            logger.error(
    ...
    for i, segment in enumerate(segments):
        ...
        try:
            ...
            if full_text is not None and word_timestamps is not None:
                segment_text = segment_transcript(
